...
import { select709-AValues } from '../../store/slices/709-aSlice';

interface 709-AFormProps {
  printMode?: boolean;
}

export const 709-AForm: React.FC<709-AFormProps> = ({ printMode = false }) => {
  const values = useSelector(select709-AValues);

  return (
    <div className={`form-709-a ${printMode ? 'print-mode' : ''}`}>
