...
import { selectW-3Values } from '../../store/slices/w-3Slice';

interface W-3FormProps {
  printMode?: boolean;
}

export const W-3Form: React.FC<W-3FormProps> = ({ printMode = false }) => {
  const values = useSelector(selectW-3Values);

  return (
    <div className={`form-w-3 ${printMode ? 'print-mode' : ''}`}>
