...
import { select990-TValues } from '../../store/slices/990-tSlice';

interface 990-TFormProps {
  printMode?: boolean;
}

export const 990-TForm: React.FC<990-TFormProps> = ({ printMode = false }) => {
  const values = useSelector(select990-TValues);

  return (
    <div className={`form-990-t ${printMode ? 'print-mode' : ''}`}>
