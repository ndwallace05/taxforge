...
import { select944-XValues } from '../../store/slices/944-xSlice';

interface 944-XFormProps {
  printMode?: boolean;
}

export const 944-XForm: React.FC<944-XFormProps> = ({ printMode = false }) => {
  const values = useSelector(select944-XValues);

  return (
    <div className={`form-944-x ${printMode ? 'print-mode' : ''}`}>
