...
import { selectW-3SSValues } from '../../store/slices/w-3ssSlice';

interface W-3SSFormProps {
  printMode?: boolean;
}

export const W-3SSForm: React.FC<W-3SSFormProps> = ({ printMode = false }) => {
  const values = useSelector(selectW-3SSValues);

  return (
    <div className={`form-w-3ss ${printMode ? 'print-mode' : ''}`}>
