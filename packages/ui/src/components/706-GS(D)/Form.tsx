...
import { select706-GS(D)Values } from '../../store/slices/706-gs(d)Slice';

interface 706-GS(D)FormProps {
  printMode?: boolean;
}

export const 706-GS(D)Form: React.FC<706-GS(D)FormProps> = ({ printMode = false }) => {
  const values = useSelector(select706-GS(D)Values);

  return (
    <div className={`form-706-gs(d) ${printMode ? 'print-mode' : ''}`}>
