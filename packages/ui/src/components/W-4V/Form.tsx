...
import { selectW-4VValues } from '../../store/slices/w-4vSlice';

interface W-4VFormProps {
  printMode?: boolean;
}

export const W-4VForm: React.FC<W-4VFormProps> = ({ printMode = false }) => {
  const values = useSelector(selectW-4VValues);

  return (
    <div className={`form-w-4v ${printMode ? 'print-mode' : ''}`}>
