...
import { selectW-4SValues } from '../../store/slices/w-4sSlice';

interface W-4SFormProps {
  printMode?: boolean;
}

export const W-4SForm: React.FC<W-4SFormProps> = ({ printMode = false }) => {
  const values = useSelector(selectW-4SValues);

  return (
    <div className={`form-w-4s ${printMode ? 'print-mode' : ''}`}>
