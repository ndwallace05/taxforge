...
import { select706-NAValues } from '../../store/slices/706-naSlice';

interface 706-NAFormProps {
  printMode?: boolean;
}

export const 706-NAForm: React.FC<706-NAFormProps> = ({ printMode = false }) => {
  const values = useSelector(select706-NAValues);

  return (
    <div className={`form-706-na ${printMode ? 'print-mode' : ''}`}>
