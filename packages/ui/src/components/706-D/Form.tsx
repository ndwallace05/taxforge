...
import { select706-DValues } from '../../store/slices/706-dSlice';

interface 706-DFormProps {
  printMode?: boolean;
}

export const 706-DForm: React.FC<706-DFormProps> = ({ printMode = false }) => {
  const values = useSelector(select706-DValues);

  return (
    <div className={`form-706-d ${printMode ? 'print-mode' : ''}`}>
