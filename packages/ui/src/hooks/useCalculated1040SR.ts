...
export const useCalculated1040SR = () => {
  ...
  return useMemo(() => {
    ...
    calculated['1040SR_line_11'] = Math.max(0, 
      calculated['1040SR_line_9'] - (formData['1040SR_line_10'] || 0)
    );
    
    // Line 14: Total deductions
    calculated['1040SR_line_14'] = (
      (formData['1040SR_line_12'] || 0) + // Standard/itemized deduction
      (formData['1040SR_line_13'] || 0)   // QBI deduction
    );
    
    // Line 15: Taxable income
    calculated['1040SR_line_15'] = Math.max(0, 
      calculated['1040SR_line_11'] - calculated['1040SR_line_14']
    );
    
    // Line 16: Tax (using tax brackets)
    calculated['1040SR_line_16'] = calculateTax(
      calculated['1040SR_line_15'], 
      formData['1040SR_filing_status']
    );
    
    // Line 18: Total tax before credits
    calculated['1040SR_line_18'] = (
      calculated['1040SR_line_16'] + 
      (formData['1040SR_line_17'] || 0) // Amount from Schedule 2
    );
    
    // Line 21: Total credits
    calculated['1040SR_line_21'] = (
      (formData['1040SR_line_19'] || 0) + // Child tax credit
      (formData['1040SR_line_20'] || 0)   // Amount from Schedule 3
    );
    
    // Line 22: Tax after credits
    calculated['1040SR_line_22'] = Math.max(0, 
      calculated['1040SR_line_18'] - calculated['1040SR_line_21']
    );
    
    // Line 24: Total tax
    calculated['1040SR_line_24'] = (
      calculated['1040SR_line_22'] + 
      (formData['1040SR_line_23'] || 0) // Other taxes
    );
    
    // Line 29: Total payments
    calculated['1040SR_line_29'] = (
      (formData['1040SR_line_25a'] || 0) + // Federal withholding
    ...
    calculated['1040SR_line_30'] = Math.max(0, 
      calculated['1040SR_line_29'] - calculated['1040SR_line_24']
    );
    
    // Line 33: Amount owed (if tax > payments)
    calculated['1040SR_line_33'] = Math.max(0, 
      calculated['1040SR_line_24'] - calculated['1040SR_line_29']
    );
    
    // Line 35: Amount you owe (including penalties)
    calculated['1040SR_line_35'] = (
      calculated['1040SR_line_33'] + 
      (formData['1040SR_line_34'] || 0) // Estimated tax penalty
    );
    
    // Line 37: Total amount owed
    calculated['1040SR_line_37'] = (
      calculated['1040SR_line_35'] + 
      (formData['1040SR_line_36'] || 0) // Additional penalty
    );
    
    // Line 38: Refund amount (after applied amounts)
    calculated['1040SR_line_38'] = Math.max(0,
      calculated['1040SR_line_30'] - 
    ...
    calculated['1040SR_standard_deduction_with_adjustments'] = getStandardDeductionForSeniors(
      formData['1040SR_filing_status'],
      formData['1040SR_age_65_or_older'],
      formData['1040SR_spouse_age_65_or_older'],
      formData['1040SR_blind'],
      formData['1040SR_spouse_blind']
    );
    
    // Social Security taxability calculation (common for seniors)
    calculated['1040SR_social_security_taxable_amount'] = calculateSocialSecurityTaxable(
      formData['1040SR_line_6a'] || 0, // Total SS benefits
    ...
    // Marginal tax rate
    calculated['1040SR_marginal_tax_rate'] = getMarginalTaxRate(
      calculated['1040SR_line_15'],
      formData['1040SR_filing_status']
    );
...
const getStandardDeductionForSeniors = (
  filingStatus: string,
...
const calculateSocialSecurityTaxable = (
  ...
  const thresholds = {
    ...
    married_filing_separately: { first: 0, second: 0 }, // Special rules apply
  ...
  if (provisionalIncome <= threshold.first) {
    ...
    return Math.min(
      totalSSBenefits * 0.5,
      (provisionalIncome - threshold.first) * 0.5
    );
  } else {
    // Up to 85% of benefits may be taxable
    const fiftyPercentAmount = Math.min(
      totalSSBenefits * 0.5,
      (threshold.second - threshold.first) * 0.5
    );
    const eightyFivePercentAmount = Math.min(
      totalSSBenefits * 0.85,
      fiftyPercentAmount + ((provisionalIncome - threshold.second) * 0.85)
    );
